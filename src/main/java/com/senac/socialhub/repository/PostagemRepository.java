...
public interface PostagemRepository extends JpaRepository<Postagem, Long> {
    ...
    @Query("SELECT p FROM Postagem p WHERE LOWER(p.titulo) LIKE LOWER(CONCAT('%', :titulo, '%')) ORDER BY p.dataCriacao DESC")
    List<Postagem> findByTituloContainingIgnoreCaseOrderByDataCriacaoDesc(@Param("titulo") String titulo);

    @Query("SELECT p FROM Postagem p WHERE LOWER(p.conteudo) LIKE LOWER(CONCAT('%', :conteudo, '%')) ORDER BY p.dataCriacao DESC")
